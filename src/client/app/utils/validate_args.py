...
class Validate_Args():
    ...
    def validate_args(args):
        ...
        schema = Schema({
            ...
            '<customer-id>': Or(None, And(Use(int)), error='Invalid input for `<customer-id>`. The event ID can only be an integer number. Example: `3`.'),
        ...
        if args.get('--date') is not None and args.get('--sale-start-date') is not None:
            ...
            if sale_start_date > event_date:
                raise SchemaError(None, errors="Invalid input for `--date` or `--sale-start-date`. The event date cannot be earlier than the start date of the sale. Please check your input.")
        try:
