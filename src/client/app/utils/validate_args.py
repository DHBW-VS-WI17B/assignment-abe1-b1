...
class Validate_Args():
    ...
    def validate_args(args):
        ...
        schema = Schema({
            ...
            '--ticket-price': Or(None, And(Use(str), Regex(r'^\s*(?=.*[1-9])\d*(?:\.\d{1,2})?\s*$')), error='The price of the tickets can only be a positive number with tow decimal places, please do not use currency symbols e.g. 5.60'),
            ...
            '--budget': Or(None, And(Use(str), Regex(r'^\s*(?=.*[1-9])\d*(?:\.\d{1,2})?\s*$')), error='The customer budget can only be a positive number with tow decimal places, please do not use currency symbols e.g. 5.60'),
